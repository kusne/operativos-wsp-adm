...
(function () {
  ...
  async function leerIniciosGuardiaDesdeSupabase() {
    ...
    // Blindaje: en INFORMES no se usa localStorage ni guardias anteriores porque eso infla
    // el contador con pruebas/inicios viejos. Se deduplica por operativo_key y también por
    // datos visibles (horario+lugar+tipo+orden) para evitar que un mismo INICIO editado
    // aparezca como 2, 4 o más operativos.
...
${bold(`Moviles ${organismo}:`)}`)
  ...
  // ======================================================
  // ===== INFORMES EXTERNOS (wsp-informes.js) =============
  // ======================================================
  // Separación segura: el código de informes vive en wsp-informes.js
  // y se inyecta dentro de este scope para conservar acceso a helpers
  // internos de WSP sin reescribir toda la app en una sola pasada.
  try {
    if (!window.WSP_INFORMES_SOURCE) {
      throw new Error("No se cargó wsp-informes.js antes de wsp.js");
    }
    eval(window.WSP_INFORMES_SOURCE);
  } catch (e) {
    console.error("[WSP] Error cargando módulo externo de informes.", e);
    alert("No se pudo cargar el módulo de informes. Verifique que wsp-informes.js/controlsuperior.js esté cargado antes de wsp.js.");
  }
